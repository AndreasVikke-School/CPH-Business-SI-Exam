...
func (uq *UserQuery) QueryLoans() *LoanQuery {
	query := &LoanQuery{config: uq.config}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withLoans; query != nil {
		...
		for _, n := range neighbors {
			fk := n.user_loans
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_loans" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_loans" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Loans = append(node.Edges.Loans, n)
		}
	}
